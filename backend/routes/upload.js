...
// New protected route for regular users
router.post('/csv-user', verifyToken, upload.single('file'), (req, res) => {
  if (!req.file) {
    return res.status(400).json({ error: 'No file uploaded' });
  }
  ...
  fs.createReadStream(filePath)
    ...
    .on('end', async () => {
      try {
        await Mobile.insertMany(results);
        fs.unlinkSync(filePath);
        res.json({ message: "CSV uploaded successfully", count: results.length, uploadedBy: req.user.email });
      } catch (error) {
        console.error(error);
        res.status(500).json({ error: "Database insert error" });
      }
